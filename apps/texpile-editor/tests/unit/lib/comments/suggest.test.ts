...
describe('a format change', () => {
	it('is the same words with other formatting, named when the words say', () => {
		...
		expect(formatChange('**x**', 'x', 'md')?.added).toEqual(['bold']);
		expect(formatChange('~~x~~', 'x', 'md')?.added).toEqual(['strikethrough']);
		expect(formatChange('*x*', 'x', 'typ')?.added).toEqual(['bold']);
		expect(formatChange('#underline[x]', 'x', 'typ')?.added).toEqual(['underline']);
	});
	...
	it('is not other words, half of a gesture, an addition or a deletion', () => {
		...
		expect(formatChange('**x', 'x', 'md')).toBeNull();
	});
});
