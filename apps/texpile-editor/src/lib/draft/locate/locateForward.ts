...
export async function locateForward(
	...
	// the daemon's box is \noindent, so an indented page paragraph reproduces only with \parindent in front;
	// a list item is indented by its list, never by that prefix
	const plain = await calibrated(false);
	if (!('bail' in plain) || listItem || plain.bail === 'cal-empty') return plain;
	const indented = await calibrated(true);
	return 'bail' in indented ? plain : indented;

	async function calibrated(indent: boolean): Promise<Cal | CalBail> {
		...
		const cal = await ctx.typesetParagraph({ text: (indent ? INDENT_PREFIX : '') + orig, hsize: W });
